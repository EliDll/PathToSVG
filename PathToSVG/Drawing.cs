...
{
    /// <summary>
    /// Handles SkiaSharp interaction and rendering
    /// </summary>
    public static class Drawing
    {
        ...
        private static ImagePiece? TryGetNext(this ImagePath path, int pieceIdx)
        {
            return pieceIdx < path.Pieces.Count - 1 ? path.Pieces[pieceIdx + 1] : null;
        }

        private static ImagePiece? TryGetPrevious(this ImagePath path, int pieceIdx)
        {
            return pieceIdx > 0 ? path.Pieces[pieceIdx - 1] : null;
        }

        private static void Draw(this ImageArc arc, SKCanvas canvas, Bounds bounds, Margin margin, SKPaint paint, int? minSample = null, int? maxSample = null)
        {
            var minIdx = Math.Max(minSample ?? 0, 0);
            var maxIdx = Math.Min(maxSample ?? arc.ImageSampleBounds.Count - 1, arc.ImageSweepSamples.Count - 1);
            for (int sampleIdx = minIdx; sampleIdx < maxIdx; sampleIdx++)
            {
                var start = arc.ImageSweepSamples[sampleIdx];
                var end = arc.ImageSweepSamples[sampleIdx + 1];
                canvas.DrawLine(start.ToSKPoint(bounds, margin), end.ToSKPoint(bounds, margin), paint);
            }
        }

        private static void Draw(this ImageLine line, SKCanvas canvas, Bounds bounds, Margin margin, SKPaint paint)
        {
            canvas.DrawLine(line.ImageStart.ToSKPoint(bounds, margin), line.ImageEnd.ToSKPoint(bounds, margin), paint);
        }

        private static SKColor BlendColors(SKColor a, SKColor b, float t = 0.5f)
        {
            return new SKColor(
        ...
        public static byte[] DrawToBytes(Path3D path, View view, Anchor anchor, DisplaySettings settings)
        {
            ...
            var debugBackdrop = false;

            int degreesPerSample = 5;
            var samplesToTangent = 90 / degreesPerSample;

            var projection = path.GetProjection(view, anchor);
            var imagePath = path.ToImagePath(projection, degreesPerSample);
            var originalPathBounds = imagePath.GetBounds();
                ...
                case HandleOverlaps.Shift:
                    {
                        ...
                        var unshiftedMaxImageRange = Math.Max(originalPathBounds.Range.Y, originalPathBounds.Range.X);
            ...
            var imageBounds = imagePath.GetBounds();
            ...
            var maxImageRange = Math.Max(imageBounds.Range.Y, imageBounds.Range.X);
            ...
            {
                Color = BlendColors(highlightPathColor, basePathColor),
                StrokeWidth = pathStrokeWidth,
                StrokeCap = SKStrokeCap.Round,
            };
            ...
            var pathHasXRange = imageBounds.Range.X > imagePath.Diameter * rangeTolerance;
            var pathHasYRange = imageBounds.Range.Y > imagePath.Diameter * rangeTolerance;
            var pathHasZRange = imageBounds.Range.Z > imagePath.Diameter * rangeTolerance;
            ...
            var canvasRect = new SKRect(0, 0, imageBounds.Range.X + margin.Left + margin.Right, imageBounds.Range.Y + margin.Top + margin.Bottom);

            using (var stream = new MemoryStream())
            {
                using (var canvas = SKSvgCanvas.Create(canvasRect, stream))
                {
                    #region Draw Backdrop (Debug)
                    var fullWidth = margin.Left + imageBounds.Range.X + margin.Right;
                    var fullHeight = margin.Top + imageBounds.Range.Y + margin.Bottom;
                    ...
                    for (int pieceIdx = 0; pieceIdx < imagePath.Pieces.Count; pieceIdx++)
                    {
                        ...
                        {
                            arc.Draw(canvas, imageBounds, margin, basePathPaint);
                        }
                        else if (piece is ImageLine line)
                        {
                            line.Draw(canvas, imageBounds, margin, basePathPaint);
                        }
                    }
                    #endregion
                    ...
                    for (int pieceIdx = 0; pieceIdx < imagePath.Pieces.Count; pieceIdx++)
                    {
                        ...
                        {
                            arc.Draw(canvas, imageBounds, margin, dependentPathPaint);
                        }
                        else if (piece is ImageLine line && line.Line3D.IsDependent)
                        {
                            line.Draw(canvas, imageBounds, margin, dependentPathPaint);
                            if (settings.HighlightOuterDimensions)
                            {
                                var prevPiece = TryGetPrevious(imagePath, pieceIdx);
                                if (prevPiece is ImageArc prevArc)
                                {
                                    var minSample = prevArc.ImageSweepSamples.Count - 1 - samplesToTangent;
                                    var maxSample = prevArc.ImageSweepSamples.Count - 1;
                                    prevArc.Draw(canvas, imageBounds, margin, dependentPathPaint, minSample, maxSample);
                                }

                                var nextPiece = TryGetNext(imagePath, pieceIdx);
                                if (nextPiece is ImageArc nextArc)
                                {
                                    var minSample = 0;
                                    var maxSample = samplesToTangent;
                                    nextArc.Draw(canvas, imageBounds, margin, dependentPathPaint, minSample, maxSample);
                                }
                            }
                        }
                    }
                    ...
                    for (int pieceIdx = 0; pieceIdx < imagePath.Pieces.Count; pieceIdx++)
                    {
                        ...
                        {
                            arc.Draw(canvas, imageBounds, margin, selectedPathPaint);
                        }
                        else if (piece is ImageLine line && line.Line3D.IsSelected)
                        {
                            line.Draw(canvas, imageBounds, margin, selectedPathPaint);
                            if (settings.HighlightOuterDimensions)
                            {
                                var prevPiece = TryGetPrevious(imagePath, pieceIdx);
                                if (prevPiece is ImageArc prevArc)
                                {
                                    var minSample = prevArc.ImageSweepSamples.Count - 1 - samplesToTangent;
                                    var maxSample = prevArc.ImageSweepSamples.Count - 1;
                                    prevArc.Draw(canvas, imageBounds, margin, selectedPathPaint, minSample, maxSample);
                                }
                                ...
                                if (nextPiece is ImageArc nextArc)
                                {
                                    var minSample = 0;
                                    var maxSample = samplesToTangent;
                                    nextArc.Draw(canvas, imageBounds, margin, selectedPathPaint, minSample, maxSample);
                                }
                            }
                        }
                    }
                    ...
                    if (debugBounds)
                    {
                        foreach (var piece in imagePath.Pieces)
                        {
                            if (piece is ImageLine line)
                            {
                                ...
                                {
                                    var pt = outerPt.ToSKPoint(imageBounds, margin);
                        ...
                        for (int pieceIdx = 0; pieceIdx < imagePath.Pieces.Count; pieceIdx++)
                        {
                            ...
                            if (currentPiece is ImageLine currentLine)
                            {
                                ...
                                if (imagePath.Pieces.Count > 1)
                                {
                                    ...
                                    var distToLeft = Math.Abs(imageCenter.X - imageBounds.Min.X);
                                    var distToRight = Math.Abs(imageCenter.X - imageBounds.Max.X);
                                    var distToCenterWidth = Math.Abs(imageCenter.X - imageBounds.Center.X);

                                    var distToTop = Math.Abs(imageCenter.Y - imageBounds.Max.Y);
                                    var distToBottom = Math.Abs(imageCenter.Y - imageBounds.Min.Y);
                                    var distToCenterHeight = Math.Abs(imageCenter.Y - imageBounds.Center.Y);
                                    ...
                                    canvas.DrawText(lineString, imageCenter.ToSKPoint(imageBounds, margin), textAlign, labelFont, lengthLabelPaint);
                                }
                                ...
                                if (nextPiece is ImageLine nextLine && nextLine.Line3D.IsValidLine3D() && currentLine.Line3D.IsValidLine3D())
                                {
                                    ...
                                    if (settings.HideAnglesModuloDeg == null || !sweepDeg.IsMultipleModulo(settings.HideAnglesModuloDeg.Value))
                                    {
                                        ...
                                        canvas.DrawText(angleString, currentLine.ImageEnd.ToSKPoint(imageBounds, margin) + new SKPoint(0, labelFont.Size * 0.5f) + new SKPoint(offset.X, -offset.Y), SKTextAlign.Center, labelFont, angleLabelPaint);
                                    }
                                }
                            }
                            else if (currentPiece is ImageArc currentArc)
                            {
                                ...
                                {
                                    var angleString = $"{sweepDeg.ToFormattedString(settings.DecimalSeparator, settings.FractionDigits)}°";
                                    canvas.DrawText(angleString, currentArc.ImageCenter.ToSKPoint(imageBounds, margin) + new SKPoint(0, labelFont.Size * 0.5f), SKTextAlign.Center, labelFont, angleLabelPaint);
                                }
                            }
                        }
                    }
                    ...
                    if (settings.DisplayMeasurements)
                    {
                        ...
                        var xLineHeight = margin.Top + imageBounds.Range.Y + halfCapWidth + totalDimensionsAxisPaint.StrokeWidth;
                        var yLineWidth = margin.Left + imageBounds.Range.X + halfCapWidth + totalDimensionsAxisPaint.StrokeWidth;
                        ...
                        if (displayX)
                        {
                            ...
                            var rangeXEnd = new SKPoint(margin.Left + imageBounds.Range.X, xLineHeight);
                            ...
                            var rangeXString = (originalPathBounds.Range.X * (float)settings.MillimetresToDisplayLengthUnit).ToFormattedString(settings.DecimalSeparator, settings.FractionDigits);

                            canvas.DrawText($"{rangeXString}{lengthSuffix}", imageBounds.Range.X * 0.5f + margin.Left, fullHeight - totalDimensionsAxisPaint.StrokeWidth, SKTextAlign.Center, labelFont, totalDimensionsAxisPaint);
                        }

                        if (displayY)
                        {
                            var rangeYStart = new SKPoint(yLineWidth, margin.Top);
                            var rangeYEnd = new SKPoint(yLineWidth, margin.Top + imageBounds.Range.Y);
                            ...
                            var rangeYString = (originalPathBounds.Range.Y * (float)settings.MillimetresToDisplayLengthUnit).ToFormattedString(settings.DecimalSeparator, settings.FractionDigits);
                        ...
                        if (displayZ)
                        {
                            ...
                            var rangeZString = (originalPathBounds.Range.Z * (float)settings.MillimetresToDisplayLengthUnit).ToFormattedString(settings.DecimalSeparator, settings.FractionDigits);
