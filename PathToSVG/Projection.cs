                ...
                case Anchor.FirstLine:
                    {
                        ...
                        for (int i = 0; i < path3D.Pieces.Count; i++)
                        {
                            ...
                            {
                                if (Util.NonZeroDistance(line.End, line.Start))
                                {
                                    bestXLineIdx = i;
                                    break;
                                }
                ...
                case Anchor.LongestLine:
                    {
                        ...
                        var longestLineLen = Util.FLOAT_EPS;
                ...
                case Anchor.LongestCollinearGroup:
                    {
                        ...
                        for (int i = 0; i < path3D.Pieces.Count; i++)
                        {
                            ...
                            if (piece is Line3D line)
                            {
                                ...
                                if (currentKeys.Any(key => Util.AreCollinear(key, lineDir)))
                                {
                                    //Append to existing group
                                    var targetKey = currentKeys.FirstOrDefault(key => Util.AreCollinear(key, lineDir));
                        ...
                        var longestSpan = Util.FLOAT_EPS;
            ...
            for (int pieceIdx = 0; pieceIdx < path3D.Pieces.Count; pieceIdx++)
            {
                ...
                if (piece3D is Line3D line3D)
                {
                    ...
                    if (pieceIdx != 0 && path3D.Pieces[pieceIdx - 1] is Arc3D startArc)
                    {
                        ...
                        var startArcWidth = Util.GetArcWidth(startArcOuterRadius, startArc.SweepDeg);
                    ...
                    if (pieceIdx != path3D.Pieces.Count - 1 && path3D.Pieces[pieceIdx + 1] is Arc3D endArc)
                    {
                        ...
                        var endArcWidth = Util.GetArcWidth(endArcOuterRadius, endArc.SweepDeg);
                    ...
                    if (Util.NonZeroDistance(imageStart2D, imageEnd2D))
                    {
